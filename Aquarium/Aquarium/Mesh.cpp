...
//        glUniform1i(glGetUniformLocation(shader.ID, ("texture_" + textures[i].GetTextureType()).c_str()), i);
...
Mesh::Mesh(const vector<Vertex>& vertices, const vector<unsigned int>& indices, const vector<TextureStruct>& textures)
{
...
Mesh::Mesh(unsigned int numVertices, std::shared_ptr <Vertex> vertices, unsigned int numIndexes, std::shared_ptr <unsigned int> indices, const vector<TextureStruct>& textures)
{
...
void Mesh::DrawBorder(const Shader& shader)
{
   // bind appropriate textures
   unsigned int diffuseNr = 1;
   unsigned int specularNr = 1;
